...
export default function StudentLiveClassPage({
  ...
  useEffect(() => {
    // Fetch Class Details (which handles enrollment and role validation)
  ...
  useEffect(() => {
    ...
    // Listen to online presence state
  ...
  // Initialize Jitsi when Jitsi details are fetched and the CDN script is loaded
  const startMeeting = () => {
    ...
    try {
      ...
      const api = new window.JitsiMeetExternalAPI("meet.jit.si", {
        ...
        configOverwrite: {
          startWithAudioMuted: true, // Muted by default to keep class disciplined
          startWithVideoMuted: true,
          prejoinPageEnabled: false, // Bypass prejoin screen for instant entry
          disableDeepLinking: true,   // Prevent forcing mobile app install on phones
          enableWelcomePage: false,
          enableClosePage: false,
          disableInviteFunctions: true,
        },
        userInfo: {
          displayName: userProfile?.name || "Student",
      ...
      // Apply password-protection flow once connected
      ...
      console.error("Failed to initialize student Jitsi meeting:", err);
      setError("Unable to launch video classroom. Please try again.");
    }
  };

  // Dynamic script loader: starts immediately when student credentials & class details are ready
  useEffect(() => {
    ...
    if (!script) {
      ...
      script.onerror = () => {
        setError("Failed to load live video engine. Please refresh the page.");
      };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-4">
        ...
        <p className="text-slate-400 font-semibold">Connecting to secure classroom session...</p>
      </div>
    );
  }
      ...
      {permissionBanner && (
        <div className="bg-amber-500/15 border-b border-amber-500/30 px-4 py-2 flex items-center justify-between text-xs text-amber-200">
          <div className="flex items-center gap-2">
            ...
            <span>
              <strong>Mic / Camera Blocked:</strong> Tap the 🔒 lock icon in your address bar and set Camera/Mic to <strong>Allow</strong>.
            </span>
          </div>
          <div className="flex items-center gap-2 shrink-0 ml-2">
            <button
              onClick={testPermissions}
              className="bg-amber-500 text-slate-950 font-bold px-2 py-0.5 rounded text-[11px]"
            >
              Retry
            </button>
            <button onClick={() => setPermissionBanner(false)} className="text-amber-400 hover:text-white">
              <X className="h-4 w-4" />
            </button>
          </div>
        </div>
      ...
      <header className="h-14 sm:h-16 shrink-0 bg-slate-900/80 border-b border-slate-800/80 px-3 sm:px-6 flex items-center justify-between backdrop-blur-xl z-20">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-xs sm:text-base font-extrabold text-white truncate max-w-[160px] sm:max-w-md">
              {classDetails?.title || "Live Lecture"}
            </h1>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <a
            ...
            title="Open full-screen in dedicated window (Recommended on mobile phones)"
          >
          ...
          <div className="hidden sm:flex items-center gap-2 bg-slate-800/60 border border-slate-700/50 px-3 py-1 rounded-full text-xs font-bold text-slate-300">
            ...
            <span>{activeUsers.length} Students</span>
          </div>
        </div>
      </header>
          ...
          <div className="flex-1 relative rounded-2xl overflow-hidden border border-slate-800/80 bg-slate-900/40 shadow-2xl flex items-center justify-center">
            ...
            {!jitsiApi && (
              <div className="text-center space-y-3 p-4">
                ...
                <p className="text-slate-400 text-sm font-semibold">Connecting to live lecture...</p>
        ...
        >
          <div className="h-12 border-b border-slate-800/80 px-4 flex items-center justify-between bg-slate-900/30 shrink-0">
            <div className="flex items-center gap-2 font-bold text-xs sm:text-sm text-slate-200">
              ...
              <span>Live Doubt Clearing Chat</span>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3.5 scrollbar-thin scrollbar-thumb-slate-800">
            {messages.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-2">
                ...
                <p className="text-slate-600 text-[11px]">Ask your doubts or say hello to the instructor!</p>
              </div>
            ) : (
              messages.map((msg) => {
          ...
          <form onSubmit={handleSendMessage} className="p-3 border-t border-slate-800/80 bg-slate-900/30 shrink-0">
            <div className="flex items-center gap-2">
              <Input
                placeholder="Ask a question or doubt..."
                value={inputText}
